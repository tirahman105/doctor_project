import { resumeStorage } from "./storage-resume.mjs";
...
if (["storage-resume","storage-anonymous-probe", "storage-anonymous-bearer-probe", "storage-remaining-v1"].includes(process.argv[2])) {
  if (!(await resumeStorage(process.env, process.argv.slice(2))))
    process.exitCode = 1;
} else
  try {
